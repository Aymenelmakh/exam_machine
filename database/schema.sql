...
-- TABLE : registrations
-- ══════════════════════════════════════════════════════════════════════════
-- TODO 1.1 — Créez cette table complètement
--
-- Champs minimum attendus :
--   id            INT UNSIGNED AUTO_INCREMENT PRIMARY KEY
--   event_id      → clé étrangère vers events(id)
--   name          VARCHAR → nom du participant
--   email         VARCHAR → email du participant
--   token         VARCHAR(64) UNIQUE → pour le lien de désinscription
--   registered_at DATETIME DEFAULT CURRENT_TIMESTAMP
--
-- Contraintes à ajouter :
--   → FOREIGN KEY (event_id) REFERENCES events(id) ON DELETE CASCADE
--   → UNIQUE KEY sur (event_id, email) → empêcher les doublons d'inscription
--
...
-- ══════════════════════════════════════════════════════════════════════════
-- Index composé pour optimiser la recherche d'événements filtrés par date ET catégorie.
CREATE INDEX idx_events_date_category ON events (event_date, category);

-- Justification :
-- La fonction `searchEvents()` permet de filtrer les événements par date et/ou par catégorie.
-- Un index composé sur `(event_date, category)` est très performant pour ce type de requête,
-- car il permet au moteur de base de données de localiser rapidement les lignes correspondant
-- aux deux critères sans avoir à scanner toute la table ou à combiner deux index distincts.
...
INSERT INTO registrations (event_id, name, email, token) VALUES
    (1, 'Yassine El Fassi', 'yassine@example.ma', 'a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2'),
    (1, 'Salma Benali', 'salma@example.ma', 'b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3'),
    (2, 'Mehdi Khalil', 'mehdi@example.ma', 'c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4'),
    (3, 'Zineb Moussaoui', 'zineb@example.ma', 'd4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5'),
    (3, 'Yassine El Fassi', 'yassine@example.ma', 'e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6');
